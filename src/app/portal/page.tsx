...
export default async function PortalOverviewPage() {
  ...
  const onboardings = allOnboardings.filter(
    (onboarding) => onboarding.claimant.id === session.claimantId,
  );

  const claimant = onboardings[0]?.claimant;
  ...
  const claimRows = (
    await Promise.all(
      onboardings.map(async (onboarding) => {
         ...
         */
        const claimantParticipant = resolved.claim.participants.find(
        ...
        const [property, documentRequests] = await Promise.all([
          getPropertyById(resolved.claim.propertyId),

          listClaimDocumentRequests(resolved.claim.id),
        ]);
  ...
  const completedClaims = claimRows.filter(
    ({ claim }) => claim.status === "paid" || claim.status === "closed",
  );

  const outstandingRequests = claimRows.flatMap((row) =>
  ...
  const confirmedRecovered = completedClaims.reduce(
  ...
  const nextDocumentAction = outstandingRequests.slice().sort((left, right) => {
    const leftDue = left.request.dueBy ?? "9999-12-31";

    const rightDue = right.request.dueBy ?? "9999-12-31";

    return leftDue.localeCompare(rightDue);
  })[0];

  const preferredName =
    claimant?.preferredName?.trim() || claimant?.legalName?.trim();
    ...
    <div className="space-y-8">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl">
          {firstName ? `Welcome back, ${firstName}` : "My Duequity"}
        </h1>

        <p className="mt-1.5 text-md text-ink-600">
          {activeClaims.length > 0
            ? `You have ${formatCount(activeClaims.length)} active ${plural(
                activeClaims.length,
                "recovery",
                "recoveries",
              )} with Duequity.`
      ...
      {nextDocumentAction ? (
        <Card elevated className="border-caution-200 bg-caution-50">
          <CardBody>
            ...
            <p className="mt-2 text-lg leading-snug font-semibold text-ink-900">
              Upload {DOCUMENT_KIND_LABEL[nextDocumentAction.request.kind]}.
            </p>

            <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-sm text-ink-600">
              <span>
                Claim{" "}
                <Identifier>{nextDocumentAction.claim.reference}</Identifier>
              </span>

              {nextDocumentAction.request.dueBy && (
                <span>
                  Requested by {formatDate(nextDocumentAction.request.dueBy)}
                </span>
              )}

              {nextDocumentAction.request.status === "overdue" && (
                <Badge tone="critical">Overdue</Badge>
              )}
            </div>

            {nextDocumentAction.request.guidance && (
        ...
        <Callout tone="positive" title="Nothing is needed from you right now">
          <p>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Stat
          ...
          value={formatCount(activeClaims.length)}
          context={
            activeClaims.length > 0 ? "Currently in progress" : "None currently"
          }
        />

        <Stat
          label="Items needed"
          value={formatCount(outstandingRequests.length)}
          tone={outstandingRequests.length > 0 ? "caution" : "default"}
          context={
        ...
        <Stat
          label="Completed"
          value={formatCount(completedClaims.length)}
          tone={completedClaims.length > 0 ? "positive" : "default"}
          context="Paid or closed recoveries"
        />

        <Stat
          label="Confirmed recovered"
          value={<Amount cents={confirmedRecovered} size="lg" whole />}
          tone={confirmedRecovered > 0 ? "positive" : "default"}
      ...
      <section>
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="eyebrow text-ink-500">Recoveries</p>

            <h2 className="mt-1 text-xl">Your claims</h2>
          </div>
        ...
        {claimRows.length === 0 ? (
          ...
          <div className="mt-4 space-y-4">
            {claimRows.map(({ claim, property, documentRequests }) => {
              ...
              const counselEngaged =
                claim.attorneyAssignment?.status === "engaged";
                ...
                <Card key={claim.id}>
                  <CardHeader
                    ...
                    title={
                      ...
                      >
                        {property?.address.line1 ?? claim.reference}
                      </Link>
                    }
                    description={
                      property
                  ...
                  <CardBody>
                    <div className="grid gap-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
                      <div>
                        <p className="text-xs text-ink-500">Claim</p>
                    ...
                    {openRequests.length > 0 && (
                      <div className="mt-4 rounded-md border border-caution-200 bg-caution-50 px-3.5 py-3">
                        <p className="text-sm font-semibold text-ink-900">
                          ...
                          {plural(openRequests.length, "document")} needed
                        </p>
      ...
      <Callout tone="neutral" title="Your recovery information">
        <p>
