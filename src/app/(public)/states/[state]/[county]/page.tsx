 ...
 *
 * This page exists for a specific trust reason: it hands a visitor everything they
 * need to pursue the claim without Duequity, including the agency's own phone
 * number and claim form. The free option is never hidden, and a page that
 * publishes the agency's own contact details is the strongest form of that
 * commitment.
 *
...
function disclosureSourceLabel(source: string): string {
  switch (source) {
...
export async function generateMetadata({
  ...
  const jurisdiction = await findPublicJurisdiction(state, county);

  if (!jurisdiction) return { title: "Jurisdiction not found" };

  return {
    title: `${jurisdictionTitle(jurisdiction)} surplus funds`,
    description: `How surplus funds are claimed in ${jurisdictionTitle(jurisdiction)}: the agency that holds them, required documents, the claim deadline, and how to file directly at no cost.`,
  };
}
...
export default async function CountyPage({
  ...
  const title = jurisdictionTitle(jurisdiction);
    ...
    <>
      <Section tone="ink" size="sm">
        <Container>
          <Breadcrumbs
            ...
            trail={[
              { href: "/states", label: "Where we operate" },
              {
                ...
                label: jurisdiction.stateName,
              },
              { label: jurisdiction.county ?? "Statewide" },
            ]}
          />
          ...
          <div className="mt-5">
            <Badge tone={COVERAGE_TONE[jurisdiction.coverage]} size="md">
          ...
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_22rem] lg:gap-14">
            <div className="min-w-0">
                    ...
                    <DataList columns={2}>
                      ...
                      <DataItem label="Claim window">
                        {jurisdiction.claimDeadlineDays !== undefined ? (
                          <>
                            {jurisdiction.claimDeadlineDays} days from the sale
                            date
                      ...
                      <DataItem label="Controlling authority">
                        {jurisdiction.statuteReference ?? <NotRecorded />}
                      </DataItem>
                      <DataItem label="Power of attorney accepted">
                        {jurisdiction.powerOfAttorneyAccepted ? "Yes" : "No"}
                      </DataItem>
                      ...
                      <DataItem label="Estate required if owner deceased" span>
                      ...
                      {jurisdiction.paymentRoutingNote && (
                        <DataItem label="How payment is issued" span>
                          {jurisdiction.paymentRoutingNote}
                        </DataItem>
                      )}
                    </DataList>
              ...
              <div className="mt-10">
                ...
                <Card className="mt-5">
                  <CardBody>
                    {jurisdiction.requiredDocuments.length > 0 ? (
                      <Checklist
                        items={jurisdiction.requiredDocuments.map((kind) => ({
                          key: kind,
                ...
                <Card className="mt-5">
                  <CardBody>
                    ...
                    <p className="mt-4 text-sm text-ink-500">
                      See{" "}
                      <TextLink href="/fees">how Duequity fees work</TextLink>{" "}
              ...
              {/* ------------------------------------------------ disclosures */}
              {jurisdiction.disclosures.length > 0 && (
                <div className="mt-10">
                  <SectionIntro
                    eyebrow="Disclosures"
                  ...
                  <Card className="mt-5">
                    <CardBody>
                      <ul className="space-y-3">
                        {jurisdiction.disclosures.map((disclosure) => (
                          <li key={disclosure.key} className="flex gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="mt-1 text-2xs text-ink-500">
                                {disclosureSourceLabel(disclosure.source)}
            ...
            <aside className="min-w-0 space-y-6">
                  ...
                  <DataList>
                    ...
                    {jurisdiction.agencyPhone && (
                      <DataItem label="Telephone">
                        <a
                          href={`tel:+1${jurisdiction.agencyPhone.replace(/\D/g, "")}`}
                    ...
                    {jurisdiction.agencyAddress && (
                      <DataItem label="Mailing address">
                        <span className="block">
                          {jurisdiction.agencyAddress.line1}
                        </span>
                        <span className="block">
                          {jurisdiction.agencyAddress.city},{" "}
                          {jurisdiction.agencyAddress.state}{" "}
                          {jurisdiction.agencyAddress.postalCode}
                        </span>
                      </DataItem>
                    )}
                    {jurisdiction.agencyWebsite && (
                      <DataItem label="Website">
                        <TextLink href={jurisdiction.agencyWebsite} external>
                          {new URL(jurisdiction.agencyWebsite).hostname}
                        </TextLink>
                      </DataItem>
                    )}
                    {jurisdiction.claimFormUrl && (
                      <DataItem label="Claim form">
                        <TextLink href={jurisdiction.claimFormUrl} external>
              ...
              <Card inset>
                <CardBody>
                  <p className="eyebrow text-ink-500">Compliance record</p>
                  <DataList className="mt-2">
                    ...
                    <DataItem label="Rule package">
                      Version {jurisdiction.packageVersion}
                    </DataItem>
                  </DataList>
                ...
                <Card>
                  <CardBody>
                    ...
                    <p className="mt-1.5 text-sm text-ink-600">
                      Search is free and asks for no personal information.
                    </p>
                    <ButtonLink
                      href="/check"
                      variant="primary"
