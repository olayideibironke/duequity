...
import { assessDeadline, evaluateIntakeGate } from "@/domain/compliance";
import {
  OPPORTUNITY_STATUS,
  RISK_FLAG_LABEL,
  SALE_TYPE_LABEL,
...
import { formatCents, formatDate, formatCount } from "@/lib/format";
...
interface IntakeGateSummary {
  outcome: "permitted" | "conditional" | "blocked";
  reason: string;
}
...
function propertyFor(
  ...
  return propertyById.get(opportunity.propertyId);
}
...
function intakeGateFor(
  ...
  return evaluateIntakeGate(jurisdiction);
}
...
function isPersistentlyConverted(
  ...
  return (
    ...
    Boolean(opportunity.convertedClaimId) ||
    String(opportunity.status) === "converted"
  );
}

function hasBlockingFlag(opportunity: Opportunity): boolean {
...
function opportunitiesForFilter(
  ...
  if (filterStatus === "converted") {
    return all.filter((opportunity) =>
      isPersistentlyConverted(opportunity, convertedOpportunityIds),
    );
  }
  ...
  if (filterStatus === "open") {
    return active.filter(
      (opportunity) => String(opportunity.status) !== "disqualified",
    );
  }

  if (filterStatus === "confirmed") {
    return active.filter((opportunity) =>
      Boolean(opportunity.confirmedSurplus),
    );
  }
  ...
  if (filterStatus === "blocked") {
    return active.filter(
      (opportunity) =>
        hasBlockingFlag(opportunity) ||
        intakeGateFor(jurisdictionById, opportunity).outcome === "blocked",
    );
  }

  return active.filter(
    (opportunity) => String(opportunity.status) === filterStatus,
  );
}
...
export default async function ProOpportunitiesPage({
  ...
  const [allOpportunities, properties, conversions, rulePackages] =
    await Promise.all([
      listOpportunities(),
      listProperties(),
  ...
  const conversionByOpportunityId = new Map<string, PersistedConversionSummary>(
    conversions.map((conversion) => [
      conversion.opportunityId,
      {
        ...
        claimReference: conversion.claimReference,
      },
    ]),
  );

  const convertedOpportunityIds = new Set(conversionByOpportunityId.keys());

  const filtered = opportunitiesForFilter(
    allOpportunities,
    status,
  ...
  const filters = filterDefinitions.map((filter) => ({
    href: `/pro/opportunities?status=${filter.key}`,

    label: filter.label,

    count: opportunitiesForFilter(
      allOpportunities,
      filter.key,
      convertedOpportunityIds,
      jurisdictionById,
    ...
    <div className="min-w-0 space-y-5">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="min-w-0">
          ...
          <h1 className="mt-1.5 text-2xl">Opportunities</h1>

          <p className="mt-1 max-w-3xl text-sm text-ink-600">
            Researched surplus opportunities before a person becomes a claimant.
            Converted opportunities remain visible for traceability but leave
            the active opportunity queue.
          </p>
        </div>
      </div>

      <FilterLinks filters={filters} label="Filter opportunities" />
      ...
      <Card className="min-w-0 overflow-hidden">
        ...
        >
          <p className="tnum text-sm text-ink-600">
            <span className="text-xs text-ink-500">Identified value </span>
          ...
          <EmptyState
            ...
            title={
              allOpportunities.length === 0
                ? "No production opportunities yet"
                : "No opportunities match this filter"
            }
            description={
              allOpportunities.length === 0
                ? "The production pipeline is empty. New researched opportunities will appear here when they are recorded."
                : "Adjust the filter above, or open the full pipeline to see every recorded opportunity."
            }
          />
        ) : (
          <>
            ...
            <div className="hidden min-w-0 xl:block">
                    ...
                    <Table caption="Opportunity pipeline with reference, property, status, value, deadline and assignment">
                      ...
                      <TBody>
                        {filtered.map((opportunity) => {
                          ...
                          const blocking = opportunity.flags.filter(
                            (flag) =>
                              flag.severity === "blocking" && !flag.resolvedAt,
                          );

                          const attention = opportunity.flags.filter(
                            (flag) =>
                              flag.severity === "attention" && !flag.resolvedAt,
                          );
                          ...
                          const conversion = conversionByOpportunityId.get(
                            opportunity.id,
                          );

                          const converted = isPersistentlyConverted(
                            opportunity,
                            convertedOpportunityIds,
                          );
                              ...
                              >
                                ...
                                {conversion && (
                                  <span className="mt-1 flex flex-wrap items-center gap-1.5">
                                    <Badge tone="positive">Converted</Badge>

                                    <Link
                                      href={`/pro/claims/${conversion.claimId}`}
                                      className="text-2xs font-medium text-accent-700 underline decoration-accent-300 underline-offset-2 hover:text-accent-800"
                                    >
                                  ...
                                  <>
                                    ...
                                    {conversion && (
                                      <span className="mt-1 block text-2xs text-ink-500">
                                        Claim {conversion.claimReference}
                                      </span>
                                    )}
                                  </>
                                ) : (
                              ...
                              <TD>
                                {converted ? (
                                  ...
                                  <div className="flex min-w-0 flex-wrap gap-1">
                                    ...
                                    {blocking.length + attention.length > 2 && (
                                      <span className="text-2xs text-ink-500">
                                        +
                                        {blocking.length + attention.length - 2}
                                      </span>
                                    ...
                                    {blocking.length + attention.length ===
                                      0 && (
              ...
              >
                <p className="text-xs text-ink-500">
                  Converted opportunities remain available under All and
                  Converted for audit traceability.
                </p>
              </TableFooter>
            </div>
            ...
            <div className="xl:hidden">
              <RecordList>
                {filtered.map((opportunity) => {
                  const property = propertyFor(propertyById, opportunity);
                  ...
                  return (
                    <RecordListItem
                      ...
                      facts={[
                        ...
                        {
                          label: converted ? "Workflow" : "Deadline",

                          value: converted ? "Moved to Claims" : deadline.label,
                        },
                        {
                          label: converted ? "Claim" : "Blocking flags",
                        ...
                        },
                        {
                          ...
                          value: String(opportunity.riskScore),
                        },
                      ]}
                    />
                  );
