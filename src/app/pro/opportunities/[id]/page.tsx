...
import {
  ConvertOpportunity,
} from "@/components/pro/convert-opportunity";
...
export async function generateMetadata({
  ...
  /*
   * Metadata must obey the same ownership boundary as the page.
   *
   * Do not disclose an inaccessible Opportunity reference through the browser
   * title.
   */
  if (!staffAccess.accessible) {
    return {
      title:
        "Opportunity",
    };
...
export default async function ProOpportunityDetailPage({
  ...
  /*
   * Stage 16 Opportunity ownership boundary.
   *
   * This happens BEFORE Property, commercial pricing, former-owner details,
   * notes, jurisdiction detail or conversion information are loaded.
   */
  const staffAccess =
    await resolveOpportunityStaffAccess(
      session,
  ...
  const persistedConversion =
    staffAccess.conversion;
  ...
  const convertedClaimId =
    opportunity.convertedClaimId ??
    persistedConversion?.claimId;

  const convertedClaimReference =
  ...
  const calculatedCommercialPricing =
    ...
    commercialPolicy
      ? calculateCommercialFeeQuote({
          ...
          quoteId:
            `preview-${opportunity.id}-v${commercialPolicy.version}`,
  ...
  const persistedApprovalStatus =
    persistedCommercialApproval?.approvalStatus;
  ...
  const commercialPricingSummary =
    ...
    commercialPolicy
      ? `${
          commercialQuote.selectedPercentage !==
          undefined
            ? formatPercent(
                commercialQuote.selectedPercentage,
              )
    ...
    <div className="space-y-5">
      ...
      {converted &&
        convertedClaimId && (
          <Callout
            tone="positive"
            title="Opportunity converted to claim"
          >
            <p>
            ...
            {persistedConversion && (
              <p className="mt-2 text-xs text-ink-600">
                Conversion recorded{" "}
                {formatTimestamp(
                  persistedConversion.convertedAt,
                )}
                .
              </p>
            )}
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="min-w-0 space-y-5">
          <Card>
            ...
            <CardBody>
              <FigureRow
                label="Sale proceeds"
                note={
                  opportunity.sale.salePrice.basis
                }
              >
                <MoneyInline
                  fact={
                    opportunity.sale.salePrice
                  }
                />
              </FigureRow>
              ...
              <FigureRow
                ...
                sign="subtract"
                note={
                  opportunity.sale.debtSatisfied.basis
                }
              >
                <MoneyInline
                  fact={
                    opportunity.sale.debtSatisfied
                  }
                />
              </FigureRow>
              ...
              {opportunity.sale.taxesOwed && (
                <FigureRow
                  ...
                  sign="subtract"
                  note={
                    opportunity.sale.taxesOwed.basis
                  }
                >
                  <MoneyInline
                    fact={
              ...
              {opportunity.sale.saleCosts && (
                <FigureRow
                  ...
                  sign="subtract"
                  note={
                    opportunity.sale.saleCosts.basis
                  }
                >
                  <MoneyInline
                    fact={
              ...
              {opportunity.sale.juniorLiens && (
                <FigureRow
                  ...
                  sign="subtract"
                  note={
                    opportunity.sale.juniorLiens.basis
                  }
                >
                  <MoneyInline
                    fact={
              ...
              <DataList columns={2}>
                ...
                <DataItem label="Data quality">
                  <StatusBadge
                    status={
                      DATA_QUALITY[
                        opportunity.provenance.quality
                      ]
                    }
                  />
                </DataItem>
                ...
                <DataItem label="Source date">
                  {formatDate(
                    opportunity.provenance.sourceDate,
                  )}
                </DataItem>
          ...
          <Card>
            ...
            <CardBody flush>
              {opportunity.priorOwners.length ===
              0 ? (
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="mt-1 flex flex-wrap items-center gap-2">
                              <Tag>
                                {
                                  OWNER_KIND_LABEL[
                                    priorOwner.ownerKind
                                  ]
                                }
                              </Tag>

                              {priorOwner.ownershipShare !==
                                undefined && (
                                <Tag>
                                  {Math.round(
                                    priorOwner.ownershipShare *
            ...
            <CardBody>
              {!jurisdiction ? (
                <Callout tone="critical">
                  <p>
                    Pricing is unavailable until the jurisdiction rule is approved.
                  </p>
                </Callout>
              ) : !commercialPolicy ? (
                <Callout tone="critical">
                  <p>
                    No current approved Duequity fee policy matches this jurisdiction, sale type and custodian.
                  </p>
                </Callout>
              ) : !calculatedCommercialPricing ? (
                <Callout tone="critical">
                  <p>
                    Commercial pricing could not be evaluated.
                  </p>
                </Callout>
                ...
                <Callout tone="critical">
                  <p>
                    No active pricing tier covers this recovery amount.
                  </p>
                </Callout>
              ) : (
                <DataList columns={2}>
                  <DataItem label="Recovery basis">
                    {commercialQuote.recoveryBasis ===
                    "confirmed"
                      ? "Confirmed"
                      : "Estimated"}{" "}
                    {formatCents(
                  ...
                  <DataItem label="Projected claimant net">
                    <span className="font-semibold text-ink-900">
                      {formatCents(
                        commercialQuote.projectedClaimantNet,
                      )}
                    </span>
                  </DataItem>

                  <DataItem label="Pricing status">
                    {commercialWorkflowReason ??
                      commercialGate.reason}
                  </DataItem>
                </DataList>
              )}
          ...
          <Card>
            ...
            <CardBody>
              ...
              !legalRule ? (
                <Callout tone="critical">
                  <p>
                    A legal lane cannot be projected until an approved jurisdiction rule exists.
                  </p>
                </Callout>
              ) : (
                <>
                  ...
                  >
                    ...
                    <DataItem label="Jurisdiction lane floor">
                      <LegalLaneBadge
                        lane={
                          projected.jurisdictionFloor
                        }
                      />
                    </DataItem>
                  </DataList>

                  {projectedFlags.length >
                    0 && (
                    <>
          ...
          <Card elevated>
            ...
            <CardBody>
              ...
              convertedClaimId ? (
                ...
                >
                  <p>
                    An approved jurisdiction rule is required before this opportunity can be converted.
                  </p>
                </Callout>
              ) : (
                <ConvertOpportunity
                  ...
                  blockingFlags={blockingFlags.map(
                    (
                      flag,
                    ) => ({
                      label:
                  ...
                  commercialPricingGate={
                    commercialWorkflowGate
                  }
                  commercialPricingReason={
                    commercialWorkflowReason
                  }
                  commercialPricingViable={
                    commercialQuote?.viabilityStatus ===
                    "viable"
                  }
                  commercialPricingApproved={
                    commercialPricingApproved
