...
begin
  ...
  if not found then
    raise exception 'authority review not found for recovery settlement'
      using errcode='23503';
  end if;

  if v_review.status not in ('recovered','closed')
     or v_review.recovered_at is null
     or v_review.recovered_amount_cents is null then
    raise exception 'recovery settlement requires a durable recovered authority review'
      using errcode='42501';
  end if;
  ...
  if new.claim_id is distinct from v_review.claim_id
    ...
    raise exception 'recovery settlement does not match authority recovery provenance'
      using errcode='42501';
  end if;
  ...
  if not found then
    raise exception 'claimant onboarding not found for recovery settlement'
      using errcode='23503';
  end if;
  ...
  if new.commercial_quote_id is distinct from v_onboarding.commercial_quote_id
    ...
    raise exception 'recovery settlement does not match claimant commercial provenance'
      using errcode='42501';
  end if;

  select * into v_quote
  from public.commercial_fee_quotes
  where quote_id = new.commercial_quote_id;

  if not found then
    raise exception 'commercial quote not found for recovery settlement'
      using errcode='23503';
  end if;

  if v_quote.approval_status <> 'locked'
     or v_quote.locked_at is null
     or v_quote.snapshot_hash is distinct from v_onboarding.commercial_snapshot_hash then
    raise exception 'recovery settlement requires matching locked commercial provenance'
      using errcode='42501';
  end if;
  ...
  if new.payment_route is distinct from nullif(btrim(v_package.snapshot ->> 'paymentRoute'),'')
    ...
    raise exception 'recovery settlement does not match frozen payment-route provenance'
      using errcode='42501';
  end if;
  ...
  if new.fee_model is distinct from v_quote.fee_model
    ...
    raise exception 'recovery settlement fee terms do not match locked quote'
      using errcode='42501';
  end if;
  ...
  if new.gross_recovery_cents = 0 then
    ...
    v_base_fee :=
      pg_catalog.round(
        new.gross_recovery_cents::numeric *
    ...
    raise exception 'unsupported locked fee model'
      using errcode='22023';
  end if;
  ...
  v_expected_fee :=
    greatest(
      ...
      least(
        v_expected_fee,
        new.gross_recovery_cents
      )
    );

  v_expected_status :=
    case
      when v_expected_fee = 0 then 'no_fee_due'
  ...
  if new.calculated_service_fee_cents is distinct from v_expected_fee
    ...
    raise exception 'recovery settlement fee calculation does not match locked terms and actual recovery'
      using errcode='42501';
  end if;

  if new.status is distinct from v_expected_status then
    raise exception 'new recovery settlement has invalid opening status'
      using errcode='42501';
  end if;
