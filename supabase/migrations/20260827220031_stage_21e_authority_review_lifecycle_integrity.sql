...
declare
  ...
  v_summary text;
  v_detail jsonb;
  v_active_information_requests bigint;
begin
  ...
  if p_action = 'authority_review_started' then
    ...
    if v_review.acknowledged_at is null or p_occurred_at < v_review.acknowledged_at then
      raise exception 'authority review start cannot precede authority acknowledgment' using errcode = '22023';
    end if;
    ...
    select count(*) into v_active_information_requests
    from public.claim_authority_information_requests
    where authority_review_id = v_review.id
      and status in ('open','responded');

    if v_active_information_requests > 0 then
      raise exception 'authority approval is blocked while authority information requests remain unresolved' using errcode = '42501';
    end if;
    ...
    if v_active_information_requests > 0 then
      raise exception 'authority denial is blocked while authority information requests remain unresolved' using errcode = '42501';
    end if;
...
begin
  ...
  if v_review.acknowledged_at is null or p_requested_at < v_review.acknowledged_at then
    raise exception 'authority information request cannot precede authority acknowledgment' using errcode = '22023';
  end if;
...
begin
  ...
  select * into v_review
  from public.claim_authority_reviews
  where id = v_request.authority_review_id
  for update;

  if not found then
    raise exception 'authority review not found for information request' using errcode = 'P0002';
  end if;

  if v_review.status <> 'additional_information_required' then
    raise exception 'authority information response is not permitted from the current review state' using errcode = '42501';
  end if;
  ...
  if p_responded_at < v_request.requested_at then
    raise exception 'authority information response cannot precede the information request' using errcode = '22023';
  end if;
...
begin
  ...
  if v_review.status <> 'additional_information_required' then
    raise exception 'authority information request cannot be resolved from the current review state' using errcode = '42501';
  end if;
  ...
  if p_resolution = 'satisfied' then
    ...
    if p_occurred_at < coalesce(v_request.responded_at, v_request.requested_at) then
      raise exception 'authority information-request satisfaction cannot precede the recorded response' using errcode = '22023';
    end if;
    ...
    if p_occurred_at < v_request.requested_at then
      raise exception 'authority information-request withdrawal cannot precede the recorded request' using errcode = '22023';
    end if;
